...
export const CategoryPage = ({ category, title, description, pt = "pt-32" }: { category: string, title: string, description: string, pt?: string }) => {
  const { posts } = usePosts();
  const filteredPosts = posts.filter(post => post.category === category);
  
  return (
    <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${pt} pb-20 space-y-12`}>
      <div className="text-center mb-16">
        <h1 className="text-4xl md:text-5xl font-display font-black text-brand-navy mb-4 tracking-tight">{title}</h1>
      ...
      {filteredPosts.length > 0 ? (
        ...
        <div className="glass-card p-12 text-center text-neutral-400 italic text-lg">
...
export const Contact = () => (
  <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-64 pb-20 bg-white/80 backdrop-blur-md rounded-[40px] my-12 shadow-2xl border border-white/20 p-8 md:p-16">
    <div className="glass-card overflow-hidden flex flex-col md:flex-row max-w-4xl mx-auto shadow-2xl">
      <div className="md:w-1/2 bg-tz-black p-12 text-white flex flex-col justify-center relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="flex gap-4">
            {[
              { icon: Instagram, color: 'bg-pink-600', link: 'https://www.instagram.com/deogratius_malingula?igsh=YmZ5MmFlaHdwOW1m' },
              { icon: Facebook, color: 'bg-blue-600', link: 'https://www.facebook.com/deogratius.john.malingula' },
              { icon: MessageCircle, color: 'bg-green-500', link: 'https://wa.me/255626425950' },
            ].map((social, i) => (
              <a key={i} href={social.link} target="_blank" rel="noopener noreferrer" className={`w-12 h-12 rounded-2xl ${social.color} flex items-center justify-center hover:scale-110 transition-all duration-300`}>
      ...
      <div className="md:w-1/2 p-12 md:p-16 bg-white">
        <form className="space-y-8">
          ...
          <a href="mailto:deogratiusmalingula2005@gmail.com" className="btn-green w-full md:w-auto block text-center">
            Tuma Ujumbe
          </a>
        </form>
      </div>
...
export const SeraNaHojaPage = () => <CategoryPage category="Sera na Hoja" title="Sera na Hoja" description="Misingi ya maendeleo yetu imejikita katika sera zinazogusa maisha ya kila siku ya mwananchi." />;
export const HotubaPage = () => <CategoryPage category="Hotuba" title="Hotuba na Ujumbe" description="Hotuba na jumbe mbalimbali kwa ajili ya wananchi." />;
...
export const VideosPage = () => <CategoryPage category="Videos" title="Videos" description="Makusanyo ya video mbalimbali." />;
export const MiradiPage = () => <CategoryPage category="Miradi ya jamii na Ustawi" title="Miradi ya Jamii na Ustawi" description="Utekelezaji wa miradi ya maendeleo ya jamii na ustawi wa wananchi." />;
export const MaktabaPage = () => <CategoryPage category="Maktaba ya maarifa na teknolojia" title="Maktaba ya Maarifa na Teknolojia" description="Hifadhi ya maarifa, teknolojia na rasilimali za kijifunzia." />;
export const Mengineyo = () => {
  const { posts } = usePosts();
  const mainCategories = ['Kisiasa', 'Kijamii', 'Sera na Hoja', 'Hotuba', 'Mikakati', 'CCM', 'Miradi ya jamii na Ustawi', 'Maktaba ya maarifa na teknolojia'];
